...
int main()
{
	...
	win.set_image(matrix);
        ...
        for (int i = 0; i < face_info.size(); i++)
        {
            ...
            temp_obj.Avg_value=0.26f;
            temp_obj.Bound_style=rgb_pixel(255, 0, 0);
            win.clear_overlay();
            
            for (auto &x : data_faces)
            {
                
                if ((length(face_descriptors[0] - x.second) * length(face_descriptors[0] - x.second)) < temp_obj.Avg_value)
                {
                    temp_obj.Avg_value=length(face_descriptors[0] - x.second) * length(face_descriptors[0] - x.second);
                    temp_obj.Name_detected=x.first;
                    temp_obj.Bound_style=rgb_pixel(0, 255, 0);
                }
                
            }
            //win.set_image(matrix);
            image_window::overlay_rect orect(rect, temp_obj.Bound_style , temp_obj.Name_detected );
            win.add_overlay(orect);
            //cout << ": " << length(face_descriptors[0]) << endl;
            faces.clear();
